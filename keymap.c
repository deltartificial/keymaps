    ...
    switch (keycode) {
        case SHOW_LAYOUT:
            if (record->event.pressed) {
                /// @notice Open terminal and display layouts
                #ifdef __APPLE__
                SEND_STRING(SS_LGUI("space"));  /// (1) Open Spotlight on macOS
                SEND_STRING("terminal" SS_TAP(X_ENTER));  /// (2) Open Terminal
                #elif defined(__linux__)
                SEND_STRING(SS_LCTL(SS_LALT("t")));  /// (1) Open terminal on most Linux distros
                #else  /// @notice Windows
                SEND_STRING(SS_LGUI("r"));  /// (1) Open Run dialog
                SEND_STRING("cmd" SS_TAP(X_ENTER)); /// (2) Open Command Prompt
                #endif

                /// @notice Display layouts (works for all OS)
                SEND_STRING(
